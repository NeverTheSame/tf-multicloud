  ...
  sudo mkdir -p /server/logs/soft-suite
  ...
  # create a place for docker to keep its files
  ...
  sudo git clone https://"${github_token}"@github.com/serverOCM/soft-plotter.git
  cd soft-plotter || exit
  sudo git checkout development
  sudo cp ./nginx* /server/conf/

  sudo chmod u+x app.init.sh
  sudo sh /server/source/soft-plotter/app.init.sh prod
